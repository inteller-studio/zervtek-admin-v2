...
import { Database, Eye, TrendingUp, CheckCircle } from 'lucide-react'
...
export function MakesSEO() {
  const totalMakes = makesData.length
  const publishedMakes = makesData.filter((m) => m.status === 'published').length
  const avgSeoScore = Math.round(makesData.reduce((acc, m) => acc + m.seoScore, 0) / totalMakes)
  const totalViews = makesData.reduce((acc, m) => acc + m.pageViews, 0)
        ...
        <div className='grid gap-4 md:grid-cols-4'>
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>Vehicle makes</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>{publishedMakes}</div>
